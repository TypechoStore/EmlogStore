 ...
 * 发送邮件验证码
 */
function sendMailCode($mail,$key="mail"){
	$_SESSION['new'.$key] = $mail;
	...
	$_SESSION[$key.'code'] = strtoupper($randCode);
	
	$blogname = Option::get('blogname');
	sendmail_do(MAIL_SMTP, MAIL_PORT, MAIL_SENDEMAIL, MAIL_PASSWORD, $mail, '【'.$blogname.'】验证码', '欢迎使用'.$blogname.'验证码服务，您的验证码是：'.$_SESSION[$key.'code'],$blogname);
}
...
EOT;
	if(strpos($_SERVER['PHP_SELF'],ADMIN_DIR)!==false){
		$layuicss='<link rel="stylesheet" href="./views/ui/layui/css/layui.css"  media="all">';
	}else{
		$cssurl="./".ADMIN_DIR."/views/ui/layui/css/layui.css";
		if(defined('BLOG_URL')){
			$cssurl=BLOG_URL.ADMIN_DIR."/views/ui/layui/css/layui.css";
		}
		$layuicss='<link rel="stylesheet" href="'.$cssurl.'"  media="all">';
	}
	echo <<<EOT
	$layuicss
...
function emoFormat($t){
	...
	if(!empty($t) && preg_match_all('/\[.+?\]/',$t,$matches)){
		...
		foreach ($matches as $data) {
			if(isset($emos[$data]))
				$t = str_replace($data,'<img title="'.$data.'" src="'.BLOG_URL.ADMIN_DIR.'/views/ui/layui/images/face/'.$emos[$data].'"/>',$t);
		}
	}
	return $t;
}
